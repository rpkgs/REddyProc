...
# 1. Crop example demonstrating multiple years and user defined uStar-Seasons
...
## 1.1. Defining Seasons with different surface friction conditions 
...
## 1.2. UStar Threshold: Seasonal estimates
...
## 1.3. Uncertainty introduced by the uStar Threshold estimate: bootstrap  
...
```{r DEGeb_gapFillBoot1, cache = TRUE, message = FALSE}
...
grep("^GPP.*_f$", colnames(r), value = TRUE)
```

> sEddyProc_sMRFluxPartition

## 1.4. UStar Threshold: Change point detection
...
# ustar = EProc$sEstimateUstarScenarios(
#   ctrlUstarEst = usControlUstarEst(isUsingCPTSeveralT = TRUE),
#   seasonFactor = seasonFactor, nSample = 30L, probs = c(0.1, 0.9)
# )
